...
// ======== GERADOR DE DOCX (inline, sem subprocesso) ========

function gerarDocx(data) {
  const {
  ...
  // Cálculos
  const totalHoras  = equipe.reduce((a, m) => a + m.horas * m.quantidade, 0);
  const totalBruto  = equipe.reduce((a, m) => a + m.horas * m.quantidade * m.valorHora, 0);
  ...
  function buildHeader() {
    ...
    if (contatos.length) {
      hChildren.push(new Paragraph({ spacing: { before: 40, after: 0 }, children: contatos }));
    }
    return new Header({ children: hChildren });
  }
  ...
  function buildInfoBox() {
    ...
    return new Table({
      ...
      rows: linhas.map(([label, valor], i) => new TableRow({
        children: [
          celula([par([txt(label, { bold: true, size: 19, color: COR_PRIMARIA })])], { bg: i % 2 === 0 ? COR_CLARO : BRANCO, width: 2800 }),
          celula([par([txt(valor || '—', { size: 19 })])],                           { bg: i % 2 === 0 ? COR_CLARO : BRANCO, width: 6560 }),
        ],
      })),
    });
  }
  ...
  function buildTabelaRequisitos() {
    ...
    const rows = requisitos.map((r, i) => new TableRow({
      children: [
        celula([par([txt(String(i+1), { size: 18 })], { align: AlignmentType.CENTER })], { bg: i%2===0?BRANCO:COR_CLARO, width: 600  }),
        celula([par([txt(r.nome,      { size: 18, bold: true })])],                       { bg: i%2===0?BRANCO:COR_CLARO, width: 3200 }),
        celula([par([txt(r.descricao||'', { size: 18 })])],                               { bg: i%2===0?BRANCO:COR_CLARO, width: 5560 }),
      ],
    }));
    return new Table({ width: { size: 9360, type: WidthType.DXA }, columnWidths: [600, 3200, 5560], rows: [hRow, ...rows] });
  }
  ...
  function buildTabelaEquipe() {
    ...
    const rows = equipe.map((m, i) => {
      const sub = m.horas * m.quantidade * m.valorHora;
      ...
      return new TableRow({ children: [
        ...
        celula([par([txt(moeda(sub),          { size: 18 })], { align: AlignmentType.RIGHT  })], { bg, width: 2680 }),
      ]});
    });
    const totalRow = new TableRow({ children: [
      ...
      celula([par([txt(totalHoras+'h',   { bold: true, size: 18, color: BRANCO })], { align: AlignmentType.CENTER })],          { bg: COR_ACENTO, width: 1000 }),
    ...
    return new Table({ width: { size: 9360, type: WidthType.DXA }, columnWidths: [3200, 800, 1000, 1680, 2680], rows: [hRow, ...rows, totalRow] });
  }

  function buildTabelaFinanceiro() {
    const linhas = [
      { label: 'Custo da Equipe (base)',          valor: totalBruto,   bg: COR_CLARO },
      { label: `Impostos (${impostoPerc}%)`,       valor: vlImposto,    bg: BRANCO    },
      { label: `Pró-labore (${prolaborePerc}%)`,   valor: vlProlabore,  bg: COR_CLARO },
      { label: `Margem de Lucro (${lucroPerc}%)`,  valor: vlLucro,      bg: BRANCO    },
    ];
    const rows = linhas.map(l => new TableRow({ children: [
      celula([par([txt(l.label,      { size: 19 })])],                              { bg: l.bg, width: 6880 }),
      celula([par([txt(moeda(l.valor),{ size: 19 })], { align: AlignmentType.RIGHT })], { bg: l.bg, width: 2480 }),
    ]}));
    const totalRow = new TableRow({ children: [
    ...
    return new Table({ width: { size: 9360, type: WidthType.DXA }, columnWidths: [6880, 2480], rows: [...rows, totalRow] });
  }
  ...
  const children = [
    new Paragraph({
      ...
      children: [txt('PROPOSTA COMERCIAL', { bold: true, size: 40, color: COR_PRIMARIA })],
    }),
    new Paragraph({
  ...
  return new Document({
    styles: { default: { document: { run: { font: "Calibri", size: 20, color: COR_TEXTO } } } },
    sections: [{
      properties: { page: { size: { width: 11906, height: 16838 }, margin: { top: 1440, right: 1000, bottom: 1200, left: 1000 } } },
...
app.post('/api/gerar-orcamento', async (req, res) => {
  try {
    const doc    = gerarDocx(req.body);
    const buffer = await Packer.toBuffer(doc);
    const nome   = `proposta_${(req.body.cliente || 'cliente').replace(/\s+/g, '_')}.docx`;
...
app.listen(PORT, () => {
  ...
  console.log(`   Acesse: http://localhost:${PORT}/sistema_orcamento.html\n`);
});
